...
async def user_login(user_id: int = Path(ge=1, le=100, description="Enter User ID", example=3)) -> dict:
...
async def user_info(username: Annotated[str, Path(min_length=5, max_length=20, description="Enter username", example="Vlad")],
                    age: Annotated[int,Path(ge=18, le=120, description="Enter age", example="23")]) -> dict:
